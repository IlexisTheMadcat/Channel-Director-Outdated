...
class helper_functions():
    ...
    async def convert_to_readable(self,
                                  ctx: Context):  # It is advised to use update_directory(ctx) first. 'ctx' must meet the requirements for getting .guild and its directory.
    ...
    async def update_directory(self, ctx,
        ...
        try:
            ...
            dchannel = await cat.create_text_channel("directory",
                                                     topic="Managers: Leave this channel on top for easy access. Feel free to move or rename it.")
...
""")
                ...
                while True:
                    ...
                    if result is False:
                        ...
                        if not list(self.bot.univ.Directories[ctx.guild.id]["tree"]["root"].items()):
                            await msg.edit(content="""
This channel will have a directory under it when you create a channel using the special command that I provide to you.
Also, make sure I have access to all channels added.
You are free to move this channel, but it's best to leave on top.
