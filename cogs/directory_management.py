    ...
    async def create_channel(self, ctx: Context, directory: str, name: str):
        ...
        if ctx.guild.id in self.bot.univ.Directories.keys():
            if ctx.channel.id == self.bot.univ.Directories[ctx.guild.id]["channelID"]:
                ...
                navigation_route = []
                for path_node in d:
                    navigation_route.append('["' + path_node + '"]')

                navigation_route = ''.join(navigation_route)

                try:
                    get_item = eval("self.bot.univ.Directories[ctx.guild.id]['tree']" + navigation_route)  # get an item at an arbitrary depth, generated by eval()
                    # if len(d) == 1:
                    ...
                    try:
                        if len(d) == 1:
                            if isinstance(get_item, int):  # self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]]
                                ...
                                channel = await category.create_text_channel(f"finishing-creation",
                                                                             topic=f"Go back: {dchannel.mention}; Name: \"{name}\"")
                        ...
                        await self.bot.update_directory(ctx=ctx,
                                                        note=f"New channel; Name: \"{name}\"; Path: \"{directory}\".")
            ...
            if ctx.channel.id == self.bot.univ.Directories[ctx.guild.id]["channelID"]:
                ...
                try:
                    ...
                    if len(d) == 1:
                        ...
                        if isinstance(self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]][d[3]][d[4]],
                                      int):
                            raise KeyError(str(d[-1]))
                        else:
                            if name in self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]][d[3]][
                                d[4]].keys():
                                await ctx.send("A channel or category in that directory already exists.",
                                               delete_after=5)
                                return
                            self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]][d[3]][d[4]].update(
                                {name: {}})

                    await self.bot.update_directory(ctx=ctx,
                                                    note=f"New category; Name: \"{name}\"; Path: \"{directory}\".")
    ...
    async def rename_channel(self, ctx, directory, name, rename):
        ...
        if ctx.guild.id in self.bot.univ.Directories.keys():
            if ctx.channel.id == self.bot.univ.Directories[ctx.guild.id]["channelID"]:
                ...
                try:
                    if len(d) == 1:
                        self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][rename] = \
                        self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]].pop(name)
                        ...
                        self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][rename] = \
                        self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]].pop(name)
                        ...
                        self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]][rename] = \
                        self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]].pop(name)
                        ...
                        self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]][d[3]][rename] = \
                        self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]][d[3]].pop(name)
                        if isinstance(self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]][d[3]][rename],
                                      int):
                            dchannel = self.bot.get_channel(self.bot.univ.Directories[ctx.guild.id]["channelID"])
                            channel = self.bot.get_channel(
                                self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]][d[3]][rename])
                        ...
                        self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]][d[3]][d[4]][rename] = \
                    ...
                    await self.bot.update_directory(ctx=ctx,
                                                    note=f"Renamed channel \"{name}\" to \"{rename}\" in path \"{directory}\".")
            ...
            if ctx.channel.id == self.bot.univ.Directories[ctx.guild.id]["channelID"]:
                ...
                try:
                    if len(d) == 1:
                        ...
                        if name not in self.bot.univ.Directories[ctx.guild.id]["tree"][D[0]][D[1]][D[2]][D[3]][
                            D[4]].keys():
                            Branch = self.bot.univ.Directories[ctx.guild.id]["tree"][d[0]][d[1]][d[2]][d[3]][d[4]].pop(
                                name)
                        else:
                            await ctx.send(
                                "The destination directory already has a channel or category with the same name.",
    ...
    async def import_channel(self, ctx, channel, new_directory, name):
        ...
        if ctx.guild.id in self.bot.univ.Directories.keys():
            if ctx.channel.id == self.bot.univ.Directories[ctx.guild.id]["channelID"]:
                ...
                await self.bot.update_directory(ctx=ctx,
                                                note=f"Imported channel with name \"{name}\"; Path: \"{new_directory}\".")
    ...
    async def hide_channel(self, ctx, directory, name):
        ...
        if ctx.guild.id in self.bot.univ.Directories.keys():
            if ctx.channel.id == self.bot.univ.Directories[ctx.guild.id]["channelID"]:
                ...
                await self.bot.update_directory(ctx=ctx,
                                                note=f"Removed channel from directory, but was not deleted. Name: \"{name}\"; From Path: \"{directory}\".")
